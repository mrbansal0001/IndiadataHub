...
def update_comparison_states_dropdown(_):
    """
    WORKFLOW: Populate states dropdown for comparison analysis
    
    REASONING FOR DESIGN CHOICES:
    1. Dropdown Selection: Multi-select dropdown chosen for intuitive state selection
    2. Sorted Display: Alphabetical sorting for easy navigation across 35+ states
    3. Tab Input Trigger: Uses tab-content as trigger to refresh when comparison tab loads
    4. Label-Value Structure: Standard Dash dropdown format for clean UI
    
    MATHEMATICAL CONSIDERATIONS:
    - No formulas involved - purely data retrieval and formatting
    - Simple set operation: unique() removes duplicates from state names
    - Sorting algorithm: O(n log n) complexity for state name ordering
    
    DATA FLOW:
    state_data → unique states → sort alphabetically → dropdown options
    """
    try:
        # Extract unique state names from the dataset
        # This ensures only states with actual data appear in dropdown
        available_states = sorted(state_data['State name'].unique())
        
        # Convert to Dash dropdown format: [{"label": display_name, "value": internal_value}]
...
def update_comparison_attribute_dropdown(selected_category):
    """
    WORKFLOW: Hierarchical dropdown system for attribute selection
    
    REASONING FOR DESIGN CHOICES:
    1. Category-First Approach: Reduces cognitive load by grouping 82 attributes into 8 categories
    2. Dynamic Filtering: Only shows relevant attributes based on category selection
    3. Short Labels: Uses get_short_label() for better UI readability in dropdown
    4. Dependent Dropdown: Prevents overwhelming users with all 82 attributes at once
    
    MATHEMATICAL CONSIDERATIONS:
    - Set Intersection: attributes ∩ available_data_columns
    - Filtering Algorithm: O(n) where n = attributes in selected category
    - Label Transformation: String processing for display optimization
    
    CATEGORY MAPPING LOGIC:
    ATTRIBUTE_CATEGORIES = {
        "Demographics": [8 attributes],
        "Education & Literacy": [10 attributes], 
        "Employment": [7 attributes],
        ...
    }
    
    DATA FLOW:
    selected_category → ATTRIBUTE_CATEGORIES[category] → get_short_label() → dropdown options
    """
    # Early return for no selection - prevents empty dropdown errors
    if not selected_category:
        return []
    
    # Retrieve attributes for the selected category from predefined mapping
    # This ensures only relevant demographic indicators are shown
    attributes = ATTRIBUTE_CATEGORIES.get(selected_category, [])
    
    # Transform attribute names to user-friendly labels using short label function
    # Format: [{"label": "Literacy Rate", "value": "Total_Literate_pct"}]
...
def update_comparison_bar_chart(selected_states, selected_attribute, comparison_type):
    """
    WORKFLOW: Create horizontal bar chart for multi-state comparison
    
    REASONING FOR VISUALIZATION CHOICE:
    1. Horizontal Bars: Better for state name readability (long text labels)
    2. Color Gradient: Visual encoding of performance levels (red=low, green=high)
    3. Two Comparison Modes: Absolute values vs. relative to national average
    4. Interactive Elements: Hover tooltips with detailed information
    5. Sortable Display: Ascending order for clear performance ranking
    
    MATHEMATICAL FORMULAS:
    
    1. DATA AGGREGATION:
       state_avg = Σ(district_values) / n_districts_in_state
    
    2. NATIONAL AVERAGE:
       national_avg = Σ(all_state_values) / n_states
    
    3. RELATIVE COMPARISON:
       relative_value = ((state_value - national_avg) / national_avg) × 100
       
    4. COLOR NORMALIZATION:
       normalized_color = (value - min_value) / (max_value - min_value)
       
    5. PERFORMANCE CATEGORIZATION:
       if normalized > 0.8: green (top 20%)
       if 0.6 ≤ normalized ≤ 0.8: light green (good performance)
       if 0.4 ≤ normalized < 0.6: yellow (average performance)  
       if 0.2 ≤ normalized < 0.4: orange (below average)
       if normalized < 0.2: red (bottom 20%)
    
    STATISTICAL CONSIDERATIONS:
    - Handles missing data with dropna()
    - Uses groupby().mean() for robust state-level aggregation
    - Accounts for different scales between absolute and relative values
    
    DATA FLOW:
    raw_data → filter_states → group_by_state → calculate_avg → 
    apply_comparison_type → normalize_colors → create_visualization
    """
    
    # VALIDATION: Ensure minimum data requirements are met
    # Requires 2+ states for meaningful comparison
    ...
    try:
        ...
        # Filter dataset to selected states only
        comparison_data = state_data[state_data['State name'].isin(selected_states)]
        
        # Remove missing values and aggregate by state (handles multiple districts per state)
        ...
        # STEP 2: CALCULATE NATIONAL BENCHMARK
        # National average used as reference point for relative comparisons
        national_avg = state_data[selected_attribute].mean()
        
        # STEP 3: APPLY COMPARISON TYPE TRANSFORMATION
        if comparison_type == "relative":
            # RELATIVE MODE: Show deviation from national average
            # Formula: relative_performance = ((state_value - national_avg) / national_avg) × 100
            ...
            # COLOR LOGIC FOR RELATIVE: Performance-based thresholds
            # Green shades = above national average, Red shades = below average
            colors = []
            for val in y_values:
                if val > 15:
                    colors.append('#10b981')  # Strong positive (>15% above avg) - Green
                elif val > 5:
                    colors.append('#34d399')  # Moderate positive (5-15% above) - Light green
                elif val > -5:
                    colors.append('#fbbf24')  # Near average (±5%) - Yellow
                elif val > -15:
                    colors.append('#f97316')  # Moderate negative (5-15% below) - Orange
                else:
                    colors.append('#ef4444')  # Strong negative (>15% below) - Red
                    
        else:
            # ABSOLUTE MODE: Show actual percentage values
            ...
            # COLOR LOGIC FOR ABSOLUTE: Percentile-based performance ranking
            max_val = y_values.max()
            min_val = y_values.min()
            colors = []
            for val in y_values:
                # Normalize value between 0 and 1 using min-max normalization
                normalized = (val - min_val) / (max_val - min_val) if max_val != min_val else 0.5
                
                # Apply quintile-based color coding
                if normalized > 0.8:
                    colors.append('#10b981')  # Top quintile (80-100%) - Green
                elif normalized > 0.6:
                    colors.append('#34d399')  # 4th quintile (60-80%) - Light green
                elif normalized > 0.4:
                    colors.append('#fbbf24')  # 3rd quintile (40-60%) - Yellow
                elif normalized > 0.2:
                    colors.append('#f97316')  # 2nd quintile (20-40%) - Orange
                else:
                    colors.append('#ef4444')  # Bottom quintile (0-20%) - Red
        
        # STEP 4: CREATE HORIZONTAL BAR VISUALIZATION
        # Horizontal orientation chosen for better state name readability
        fig = go.Figure(data=[
            go.Bar(
                y=comparison_data['State name'],  # States on Y-axis
                x=y_values,                       # Values on X-axis
                orientation='h',                  # Horizontal bars
                marker=dict(
                    color=colors,                 # Performance-based coloring
                    line=dict(color='rgba(255,255,255,0.8)', width=2)  # White borders
                ),
                hovertemplate=hover_template,
                customdata=customdata,
                text=[f"{val:.1f}%" for val in y_values],  # Value labels on bars
                textposition='outside',           # Labels outside bars for clarity
                textfont=dict(size=12, color='#374151', weight='bold')
            )
        ])
        
        # STEP 5: APPLY STYLING AND LAYOUT CONFIGURATION
        fig.update_layout(
            ...
            title_font_color="#2d3748",
            height=400,
            margin=dict(l=120, r=50, t=60, b=50), # Extra left margin for state names
            paper_bgcolor='rgba(0,0,0,0)',
            plot_bgcolor='rgba(248,250,252,0.8)',
            font_family=FONT_FAMILY,
            xaxis=dict(
                ...
                gridwidth=1,
                showgrid=True,
                zeroline=True,                    # Important for relative comparisons
            ...
            yaxis=dict(
                ...
                showgrid=False,
                categoryorder='total ascending'   # Sort by performance
            ),
            hoverlabel=dict(
                bgcolor="white",
                bordercolor="rgba(0,0,0,0.1)",
        ...
        # STEP 6: ADD REFERENCE LINE FOR RELATIVE MODE
        if comparison_type == "relative":
            # Add zero line to show national average baseline
            fig.add_vline(
                x=0, 
                line_dash="dash", 
...
def update_comparison_radar_chart(selected_states, selected_category):
    """
    WORKFLOW: Create multi-dimensional radar chart for category-based state comparison
    
    REASONING FOR VISUALIZATION CHOICE:
    1. Radar/Spider Chart: Ideal for comparing multiple dimensions simultaneously
    2. Category-Based Analysis: Groups related attributes for holistic comparison
    3. Overlapping Traces: Visual comparison of state "profiles" across dimensions
    4. Circular Layout: Equal visual weight to all attributes in the category
    5. Fill Areas: Quick identification of strengths/weaknesses patterns
    
    MATHEMATICAL FORMULAS:
    
    1. POLAR COORDINATE SYSTEM:
       θ = 2π × (attribute_index / total_attributes)  [Angular position]
       r = attribute_value  [Radial distance from center]
    
    2. AREA CALCULATION:
       state_area = ∑(r_i × r_{i+1} × sin(θ_{i+1} - θ_i)) / 2  [Polygon area]
    
    3. COLOR TRANSPARENCY:
       fill_opacity = base_alpha / n_overlapping_states  [Prevent visual clutter]
    
    4. NORMALIZATION (if needed):
       normalized_value = (value - category_min) / (category_max - category_min)
    
    5. ATTRIBUTE SELECTION CRITERIA:
       selected_attrs = category_attrs ∩ available_data_columns
       minimum_requirement = 3 attributes per category
    
    STATISTICAL CONSIDERATIONS:
    - Handles missing data with fillna(0) for radar completeness
    - Uses category-specific scaling for fair comparison
    - Accounts for different measurement scales across attributes
    - Maintains circular continuity by repeating first value
    
    VISUAL ENCODING RATIONALE:
    - Angular position = attribute type (Education, Health, etc.)
    - Radial distance = performance level (0% to 100%)
    - Color = state identity (unique color per state)
    - Fill area = overall performance profile
    - Line thickness = emphasis on comparison boundaries
    
    DATA FLOW:
    category_selection → attribute_filtering → state_filtering → 
    polar_coordinate_mapping → overlapping_traces → radar_visualization
    """
    
    # VALIDATION: Ensure minimum requirements for meaningful radar chart
    # Requires 2+ states and minimum 3 attributes for multi-dimensional analysis
    ...
    try:
        # STEP 1: ATTRIBUTE EXTRACTION AND VALIDATION
        # Get all attributes belonging to the selected category
        attributes = ATTRIBUTE_CATEGORIES.get(selected_category, [])
        ...
        # Filter to only attributes that exist in our dataset
        # This prevents errors when category definitions don't match data
        available_attributes = []
        for attr in attributes:
            if attr in pct_cols:  # pct_cols contains all percentage columns
                available_attributes.append(attr)
        
        # RADAR CHART REQUIREMENT: Minimum 3 dimensions for meaningful comparison
        ...
        # STEP 3: STATE DATA FILTERING
        # Extract data for selected states only
        ...
        # STEP 4: CREATE RADAR CHART WITH OVERLAPPING TRACES
        fig = go.Figure()
        
        # STEP 5: COLOR PALETTE ASSIGNMENT
        # Distinct colors for each state to enable easy comparison
        # Colors chosen for accessibility and visual distinction
        state_colors = [
            '#3b82f6',  # Blue - Primary
            '#ef4444',  # Red - Alert/Secondary  
            '#10b981',  # Green - Success
            '#f59e0b',  # Amber - Warning
            '#8b5cf6',  # Purple - Info
        ]
        
        # STEP 6: GENERATE RADAR TRACE FOR EACH STATE
        for idx, (_, state_row) in enumerate(radar_data.iterrows()):
            state_name = state_row['State name']
            
            # STEP 6a: EXTRACT VALUES AND LABELS FOR CURRENT STATE
            values = []
            labels = []
            
            # Build coordinate pairs (angle, radius) for radar chart
            for attr in available_attributes:
                if pd.notna(state_row[attr]):
                    values.append(state_row[attr])
                    labels.append(get_short_label(attr))  # User-friendly labels
            
            # STEP 6b: CLOSE RADAR POLYGON
            # Radar charts require circular continuity - repeat first point
            if values:
                values.append(values[0])
                labels.append(labels[0])
            
            # STEP 6c: COLOR PROCESSING FOR TRANSPARENCY
            # Convert hex to RGB for alpha channel control
            hex_color = state_colors[idx % len(state_colors)]
            rgb = tuple(int(hex_color[i:i+2], 16) for i in (1, 3, 5))
            
            # STEP 6d: ADD RADAR TRACE WITH STYLING
            fig.add_trace(go.Scatterpolar(
                r=values,                    # Radial values (performance levels)
                theta=labels,                # Angular positions (attribute names)
                fill='toself',               # Fill the polygon area
                fillcolor=f"rgba({rgb[0]}, {rgb[1]}, {rgb[2]}, 0.1)",  # Semi-transparent fill
                line=dict(
                ...
                marker=dict(
                    color=state_colors[idx % len(state_colors)],
                    size=8,                  # Prominent markers at data points
                    line=dict(color='white', width=2)  # White outline for contrast
                ),
                name=state_name,
                # HOVER TEMPLATE: Show all attribute values for selected state
        ...
        # STEP 7: RADAR CHART STYLING AND LAYOUT
        fig.update_layout(
            ...
            polar=dict(
                bgcolor='rgba(255,255,255,0.9)',
                # RADIAL AXIS: Controls the distance from center (values)
                radialaxis=dict(
                    visible=True,
                    # DYNAMIC RANGE: Scale based on maximum value in the data
                    range=[0, max([max(radar_data[attr].fillna(0)) for attr in available_attributes]) * 1.1],
                    tickfont=dict(size=10, color="#6b7280"),
                    gridcolor="rgba(203,213,225,0.5)",  # Radial grid lines
                    gridwidth=1,
                    linecolor="rgba(203,213,225,0.7)",
                    linewidth=1
                ),
                # ANGULAR AXIS: Controls the position around the circle (attributes)
                angularaxis=dict(
                    tickfont=dict(size=11, color="#374151", weight='bold'),
                    linecolor="rgba(203,213,225,0.7)",
                    linewidth=1,
                    gridcolor="rgba(203,213,225,0.3)"  # Angular grid lines
                )
            ),
